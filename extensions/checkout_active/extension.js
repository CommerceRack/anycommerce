...
//ensure no other paypal payments in Q. could happen after cart is paypal, then user modifies cart and doesn't return to checkout
...
//if token and/or payerid is NOT set on URI, then this is either not yet a paypal order OR is/was paypal and user left checkout and has returned.
					...
					app.u.dump("THIS IS a paypal session, but needs to be reset");
...
//					app.u.dump(" -> _gaq is undefined");
					$('#globalMessaging').anymessage({'message':'<strong>Uh Oh!<\/strong> It appears you are not using the Asynchronous version of Google Analytics. It is required to use this checkout.','uiClass':'error','uiIcon':'alert'});
					r = false;					
					}
...
//					app.u.dump('BEGIN app.ext.convertSessionToOrder.callbacks.handleInventoryUpdate.onSuccess');
	...
	//				var L = app.model.countProperties(app.data[tagObj.datapointer]);
					if(!$.isEmptyObject(app.data[tagObj.datapointer]) && !$.isEmptyObject(app.data[tagObj.datapointer]['%changes']))	{
						...
						$('#globalMessaging').anymessage({'message':r,'uiIcon':'alert'});
...
//						app.u.dump(' -> email is blank');
						...
						$errorDiv.anymyssage({'message':"Please provide an email address"}).toggle(true);
...
//IE7 wants pass.val to have a value before checking length.
					...
					if(valid == 0)	{
						$fieldsetErrors.anymessage({'message':"<ul>"+errMsg+"<\/ul>"});
						}
					}
...
//					app.u.dump(' -> ship method validated');
					...
					valid = 0;
					$('#chkoutShipMethodsFieldsetErrors').anymessage({'message':"Please select a shipping method."});
...
*/
				...
				for(var i = 0; i < L; i += 1)	{
					if(app.data.cartDetail && app.data.cartShippingMethods['@methods'][i].id == app.data.cartDetail['want/shipping_id'])	{
