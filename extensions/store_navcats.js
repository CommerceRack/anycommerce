...
//				myControl.util.dump("BEGIN store_navcats.calls.categoryTree.init");
...
//if no extension is set, use this one. need to b able to override so that a callback from outside the extension can be added.
		...
		categoryDetail : {
			...
			myControl.util.dump('BEGIN myControl.ext.store_navcats.calls.categoryDetail.init ('+catSafeID+')');
				var r = 0; //will return 1 if a request is needed. if zero is returned, all data needed was in local.
...
//				myControl.util.dump('BEGIN myControl.ext.store_navcats.calls.categoryDetail.dispatch');
				var catSafeID;
//				myControl.util.dump(' -> safeid = '+catSafeID);
//				myControl.util.dump(' -> executing dispatch.');
				myControl.model.addDispatchToQ({"_cmd":"categoryDetail","safe":catSafeID,"detail":"fast","_tag" : tagObj},Q);	
				}
			},//categoryDetail

		categoryDetailMore : {
			init : function(catSafeID,tagObj,Q)	{
//				myControl.util.dump('BEGIN myControl.ext.store_navcats.calls.categoryDetailMore.init');
				var r = 0; //will return 1 if a request is needed. if zero is returned, all data needed was in local.
				tagObj = typeof tagObj !== 'object' ? {} : tagObj;
				tagObj.datapointer = 'categoryDetail|'+catSafeID;
				if(myControl.model.fetchData(tagObj.datapointer) == false)	{
...
//local is probably from a 'fast' request. go get more.
			...
			dispatch : function(catSafeID,tagObj,Q)	{
				...
				myControl.model.addDispatchToQ({"_cmd":"categoryDetail","safe":catSafeID,"detail":"more","_tag" : tagObj},Q);	
				}
			},//categoryDetailMore
		...
		categoryDetailMax : {
			init : function(catSafeID,tagObj,Q)	{
//				myControl.util.dump('BEGIN myControl.ext.store_navcats.calls.categoryDetailMax.init');
				var r = 0; //will return 1 if a request is needed. if zero is returned, all data needed was in local.
				tagObj = typeof tagObj !== 'object' ? {} : tagObj;
//whether max, more or just detail, always save to same loc.
//add here so if tagObj is passed directly into callback because data is in localStorage, the datapointer is set.
...
//				myControl.util.dump(' -> executing dispatch.');
				tagObj.detail = 'max';
				myControl.model.addDispatchToQ({"_cmd":"categoryDetail","safe":catSafeID,"detail":"max","_tag" : tagObj},Q);	
				}
			}//categoryDetailMax
...
myControl.renderFunctions.translateTemplate(myControl.data['categoryDetail|'+data.bindData.cleanValue]['@subcategoryDetail'][i],"bob_"+$tag.attr('id')+"_"+catSafeID);
*/
			},
...
//				myControl.util.dump(myControl.data['categoryDetail|'+catSafeID])
//if . is passed as catSafeID, then tier1 cats are desired. The list needs to be generated.
				var catsArray = [];
				
				if(catSafeID == '.')
					catsArray = this.getRootCats();
				else if(typeof myControl.data['categoryDetail|'+catSafeID]['@subcategoryDetail'] == 'object')
					catsArray = myControl.data['categoryDetail|'+catSafeID]['@subcategoryDetail'];
				else	{
